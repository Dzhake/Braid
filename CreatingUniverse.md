...
Braid will only read `world..` files from your mod, so you can't replace vanilla entities or menus. Tho you can edit them, and then copypaste them in your `Braid/data/entities` folder. But keep in mind other people won't have it replaced by default.

# Packaging universe

Once you want to share your universe with others, you need to package it. To do that, create folder `universes/<your universe>` anywhere, then copypaste your `Braid/universes/<your universe>/data` there. Zip that, and rename it to `package.zip`. To use it people need to put `package.zip` in `Braid/universes/<your universe>`, and use `-universe <your universe>`.
...
For more info about editing universes see other guides here. Probably you should start with [`Hotkeys`](Hotkeys.md).